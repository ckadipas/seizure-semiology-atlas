...
Deployment is not proved by a successful workflow, HTTP 200, or screenshot alone. Rebuild from the published commit, fetch both the Cloudflare Pages origin and custom-domain responses, and remove only Cloudflare's managed Pages Analytics block, hidden `/cdn-cgi/content` anchor, and request-specific challenge-loader block anchored to `window.__CF$cv$params` and `/cdn-cgi/challenge-platform/scripts/precursor/main.js`. The remaining bytes must match the generated `index.html` exactly; any other difference is a failed deployment.
