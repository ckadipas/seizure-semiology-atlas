...
| `data/brodmann_map.json` | **Source of truth** — the Brodmann areas, where each numeral sits per view, and which areas each sign localizes to (human-edited). |
...
| `generator/assets/` | Reference brain plates — the map is these, with numerals on top. |
| `generator/brain_atlas.py` | Loads the Brodmann map and answers which areas a sign localizes to (no curation). |
...
| `tools/brodmann_plate.py` | Authoring aid: cleans a reference plate into the base image used by the map. |
