...
with open(os.path.join(ROOT,"enrichment","corpus_findings.json")) as f:
    CORPUS = json.load(f)
...
def _load_optional(*parts):
    ...
    if os.path.exists(p):
        with open(p) as f:
            return json.load(f)
    return None
EVIDENCE_INDEX = _load_optional("enrichment", "evidence_index.json") or {}
META = None
...
def ledger_evidence_block(cid):
    """Render only explicit V30 sign links; every value remains in the ledger row."""
    linked = ledger_evidence_by_cardid.get(cid, [])
    if not linked:
        return ('<div class="d-row d-ev"><span class="d-label">Owner-reviewed V30 evidence</span>'
                '<span class="d-value">No exact or related V30 mapping is approved for this registry sign yet. '
                'Source-native findings remain searchable in the complete evidence ledger above.</span></div>', 0, "")
    items, search = [], []
    for entry, relation in linked:
        ...
        measure = (f'<div class="ev-measure"><strong>Source-reported measure:</strong> {esc(row["measure"])}</div>'
                   if row["source_statistic_id"] else '')
        owner = (f'<div class="ev-owner"><strong>Owner note:</strong> {esc(row["owner_note"])}</div>'
                 if row.get("owner_note") else '')
        independence = ("independent source evidence" if row["independent_evidence"]
                        else row["tabulation_status"].replace("_", " ").lower())
        items.append(
            '<li class="v30-card-evidence">'
            f'<div><span class="ev-map ev-map-{relation.lower()}">{relation}</span> '
            f'<strong>{esc(row["source_term"])}</strong> &mdash; {esc(row["claim"])}</div>'
            f'{measure}{owner}'
            '<details class="ev-trace"><summary>Source trace and context</summary>'
            f'<div><strong>Source:</strong> {esc(source["source_file"])} &middot; {esc(row["citation"])}</div>'
            f'<div><strong>Locator:</strong> {esc(row["locators"])}</div>'
            f'<div><strong>Evidence text:</strong> {esc(row["evidence_text"])}</div>'
            f'<div><strong>Population:</strong> {esc(row["population"])}</div>'
            f'<div><strong>Role:</strong> {esc(row["evidence_role"].replace("_", " ").lower())}; {esc(independence)}</div>'
            f'<div><strong>Finding identity:</strong> <code>{esc(row["source_finding_ref"])}</code></div>'
            '</details></li>')
    return ('<div class="d-row d-ev"><span class="d-label">Owner-reviewed V30 evidence</span>'
...
for r in region_order:
    ...
    for sub, signs in grouped[r].items():
        ...
        for d in signs:
            ...
            lib_chip = (f'<span class="chip lib-chip" title="Explicit owner-reviewed V30 links">&#128218; {_nsrc}</span>'
                        if _nsrc else '')
            search_str = " ".join([d["sign"], d["phase"], d["region"], d["sub"], ev_text]).lower().replace('"',"")
            ppv_block = sens_block = ""
            sens_metric = (f'<span class="d-label">Registry identity</span>'
                           f'<span class="metric-val">sign_id {d["id"]}</span>')
            spec_metric = (f'<span class="d-label">Explicit V30 links</span>'
                           f'<span class="metric-val">{_nsrc}</span>')
...
def build_v30_evidence(corpus):
    """Render every owner-approved ledger row once, without pooling or remapping."""
    role_label = {
        "PRIMARY_RESULT": "Primary result",
        "REVIEW_SYNTHESIS": "Review synthesis",
        "CITED_STUDY_RESTATEMENT": "Cited restatement",
        "EDUCATIONAL_STATEMENT": "Educational",
        "GUIDELINE_RECOMMENDATION": "Guideline",
        "CASE_OBSERVATION": "Case observation",
    }
    role_color = {
    ...
    for source in corpus["sources"]:
        for row in source["findings"]:
            ...
            mapping = row["mapping_relation"]
            mapped_ids = row["exact_sign_ids"] or row["related_sign_ids"]
            mapping_text = (f'{mapping} sign_id '+", ".join(map(str, mapped_ids)) if mapped_ids else
                            (row["prospective_concept_id"] or mapping))
            state = ("independent" if row["independent_evidence"] else
                     row["tabulation_status"].replace("_", " ").lower())
            search = " ".join(str(x or "") for x in [
                ...
                row["owner_note"], row["source_finding_ref"], source["source_file"], role, mapping_text,
            ]).lower().replace('"', "")
            owner = (f'<div class="fx-owner"><strong>Owner note:</strong> {esc(row["owner_note"])}</div>'
                     if row.get("owner_note") else "")
            measure = (f'<div><strong>Source-reported measure:</strong> {esc(row["measure"])}</div>'
                       if is_stat else "")
            rows.append(
                f'<div class="fx-row v30-row" data-metric="{role}" data-fq="{esc(search)}">'
                f'<span class="fx-m" style="background:{role_color.get(role,"#6b7280")}">{esc(role_label.get(role,role))}</span>'
                f'<span class="fx-ph">{esc(row["source_term"])}<span class="fx-reg">{esc(row["phase"])} &middot; {esc(mapping_text)}</span></span>'
                f'<span class="fx-val">{"STAT" if is_stat else "TEXT"}</span>'
                f'<span class="fx-src">{esc(source["source_file"])}<br>{esc(row["locators"])}</span>'
                f'<span class="fx-q"><strong>{esc(row["claim"])}</strong>{measure}{owner}</span>'
                '<details class="fx-context"><summary>Evidence text, population, limitations, and immutable identity</summary>'
                f'<div><strong>Evidence text:</strong> {esc(row["evidence_text"])}</div>'
                f'<div><strong>Population:</strong> {esc(row["population"])}</div>'
                f'<div><strong>Laterality/localization:</strong> {esc(row["laterality_localization"])}</div>'
                f'<div><strong>Analysis unit:</strong> {esc(row["analysis_unit"])}</div>'
                f'<div><strong>Comparator:</strong> {esc(row["comparator"])}</div>'
                f'<div><strong>Numerator / denominator:</strong> {esc(row["numerator"])} / {esc(row["denominator"])}</div>'
                f'<div><strong>Uncertainty:</strong> {esc(row["uncertainty"])}</div>'
                f'<div><strong>Limitations:</strong> {esc(row["limitations"])}</div>'
                f'<div><strong>Citation:</strong> {esc(row["citation"])}</div>'
                f'<div><strong>Evidence status:</strong> {esc(state)}</div>'
                f'<div><strong>Finding identity:</strong> <code>{esc(row["source_finding_ref"])}</code></div>'
                + (f'<div><strong>Statistic identity:</strong> <code>{esc(row["source_statistic_id"])}</code></div>' if is_stat else '')
                + '</details></div>')
    total = len(rows)
    ...
    accounting = corpus["integration_accounting"]
    return f'''<details class="frontpage-fold figures-fold" open>
<summary>Owner-reviewed V30 evidence ledger &mdash; {accounting["public_ledger_findings"]:,} public findings</summary>
<div class="fx-wrap">
  <div class="fx-intro"><strong>{accounting["owner_reviewed_findings"]:,} owner-reviewed findings</strong> &middot; <strong>{accounting["public_ledger_findings"]:,} public evidence rows</strong> &middot; <strong>{accounting["source_reported_statistics"]:,} source-reported statistics</strong> from {accounting["source_reports"]} reviewed source files. Every statistic is displayed exactly once under its immutable identity; no values are pooled. Review restatements and alternate PDF versions remain visible but are marked non-independent.</div>
...
figures_fold = build_v30_evidence(CORPUS)
...
.v30-card-evidence{margin-bottom:9px}.ev-measure,.ev-owner{margin:4px 0;color:#475569}
...
<div class="site-header">
  ...
  <p>Search the complete owner-reviewed evidence ledger or open a registry sign to see only its explicit V30 links. Source values remain study-specific and are never pooled. <span class="edu-inline">&#9888;&#65039; Educational reference &mdash; not for clinical decision-making.</span></p>
</div>
...
<div class="footer">
  <strong>Educational use:</strong> This reference is designed for teaching and self-study by epilepsy trainees. Every V30 evidence row retains its source population, analysis unit, denominator, uncertainty, evidence role, limitations, and immutable source identity; source values are not pooled or converted into clinical-performance claims. Real localization always integrates ictal EEG, imaging, neuropsychology, and history. &nbsp;|&nbsp;
  <strong>Mapping:</strong> Evidence appears on a registry card only when the V30 ledger records an explicit exact or related <code>sign_id</code> link. Unmapped source-native concepts remain visible in the evidence ledger and are not guessed into existing signs. &nbsp;|&nbsp;
